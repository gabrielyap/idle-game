...
if (len(history) >= 2):  #Runs if game has already been ran before
    ...
    print("You made " + str(abs(currData.currentGold - history[len(history) - 1].currentGold)) + " gold. You now have " + str(currData.currentGold) + " gold.")
    choice = input("Would you like to sharpen? Type \"yes\" (costs 1000 gold, +10 gold/min): ")
    if (choice == "yes"):
        choice2 = input("Do you want to spend as much gold as you can to keep sharpening? Type \"yes\" ")
        if (choice2 == "yes"):
            while (currData.currentGold >= 1000):
                sharpen(currData)
        else:
            sharpen(currData) 
history.append(currData)
